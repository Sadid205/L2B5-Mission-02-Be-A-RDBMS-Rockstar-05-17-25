...
SELECT customer_id,count(*) as total_orders,sum(total_amount) as total_spent FROM orders
...
SELECT to_char(order_date,'Month') as month,sum(total_amount) as total_amount,extract(YEAR FROM order_date) as year FROM orders
